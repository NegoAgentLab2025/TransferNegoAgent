...
if __name__ == "__main__":
    ...
    #parser.add_argument("--teacher", default="Amsterdam-B", type=str)
    ...
    rl_agent = RLAgent(args.max_round, 'rl agent', device, 3, args.algo, args.seed)

    teacher_1 = RLAgent(args.max_round, 'teacher_1', device, 3, args.algo, args.seed)
    teacher_1.load('1', args.seed)

    teacher_2 = RLAgent(args.max_round, 'teacher_2', device, 3, args.algo, args.seed)
    teacher_2.load('2', args.seed)

    teacher_3 = RLAgent(args.max_round, 'teacher_3', device, 3, args.algo, args.seed)
    teacher_3.load('3', args.seed)

    teacher_4 = RLAgent(args.max_round, 'teacher_4', device, 3, args.algo, args.seed)
    teacher_4.load('4', args.seed)

    # teacher_5 = RLAgent(args.max_round, 'teacher_5', device, 3, args.algo)
    # teacher_5.load('5')

    # teacher_6 = RLAgent(args.max_round, 'teacher_6', device, 3, args.algo)
    # teacher_6.load('6')
    ...
    allTeachers = [ponpoko_agent, atlas3_agent, agreeable_agent2018, caduceus_agent]
    allDomains = ["domain{:0>2d}".format(i) for i in range(50)]

    if args.mode == "train":
        if args.save_models:
            if args.algo == 'sac' and not os.path.exists("./sac_models/{}/mix/seed{}".format(args.oppo_type, str(args.seed))):
                os.makedirs("./sac_models/{}/mix/seed{}".format(args.oppo_type, str(args.seed)))
        ...
        logdir = './runs/runs_{}_allDomain_{}_{}_mix'.format(args.algo, args.oppo_type, str(args.seed))
        ...
        teacher_W = choose_teacher(rl_agent=rl_agent, opponent=opponent, teachers=allTeachers, domains=allDomains)
        ...
        while total_timesteps < args.max_timesteps:
            ...
            if args.domain_type == "REAL":
                ...
                domain_now = random.choice(allDomains)
                negotiation = Negotiation(max_round=args.max_round, domain_type=args.domain_type, domain_file=domain_now, train_mode=True)
            ...
            if total_timesteps < 0.9 * args.max_timesteps and total_timesteps > args.start_timesteps:
                ...
                tmp = choose_teacher(rl_agent=rl_agent, opponent=opponent, teachers=allTeachers, domains=domain_now)
                teacher_W = [0.3 * a + 0.7 * b for a,b in zip(teacher_W, tmp)]
            ...
            for i in range(1, negotiation.max_round + 1):
                ...
                if negotiation.agents_pool[current_player].__class__.__name__ == "RLAgent":
                    ...
                    if total_timesteps < args.start_timesteps:
                        ...
                        mix_factor = 1 - total_timesteps / args.max_timesteps
                        #mix_factor = - (2 / math.pi) * math.atan(8 * (total_timesteps / args.max_timesteps - 1))
            ...
            if total_timesteps < 0.9 * args.max_timesteps and total_timesteps > args.start_timesteps:
                ...
                if episode_num % 10 == 0:
                    ...
                    evaluation_reward_teacher_1 = evaluate_policy(teacher_1, opponent, domain='randomDomain', eval_episodes=10, render=False)
                    evaluation_reward_teacher_2 = evaluate_policy(teacher_2, opponent, domain='randomDomain', eval_episodes=10, render=False)
                    evaluation_reward_teacher_3 = evaluate_policy(teacher_3, opponent, domain='randomDomain', eval_episodes=10, render=False)
                    evaluation_reward_teacher_4 = evaluate_policy(teacher_4, opponent, domain='randomDomain', eval_episodes=10, render=False)
            ...
            if total_timesteps > 0.9 * args.max_timesteps:
                mix_factor = 0
            ...
            if total_timesteps >= args.start_timesteps:
                ...
                rl_agent.train(offer_replay_buffer=offer_replay_buffer, iterations=episode_timesteps, batch_size=args.batch_size,
                               discount=args.discount, tau=args.tau, policy_noise=args.policy_noise, noise_clip=args.noise_clip,
                               policy_freq=args.policy_freq, mix_factor=mix_factor, teacher_P=[])
            if episode_num_last_eval >= args.eval_freq:
                ...
                if args.save_models:
                    if args.algo == 'sac':
                        rl_agent.save(directory="./sac_models/{}/mix/seed{}".format(args.oppo_type, str(args.seed)))
                    elif args.algo == 'td3':
        ...
        # with open("mix_" + str(args.oppo_type) + "_DefensiveCharms_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_DefensiveCharms:
        #         f.write(str(i))
        #         f.write("\n")       

        # with open("mix_" + str(args.oppo_type) + "_Acquisition_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_Acquisition:
        #         f.write(str(i))
        #         f.write("\n")     
        
        # with open("mix_" + str(args.oppo_type) + "_Kitchen_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_Kitchen:
        #         f.write(str(i))
        #         f.write("\n") 
        
        # with open("mix_" + str(args.oppo_type) + "_HouseKeeping_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_HouseKeeping:
        #         f.write(str(i))
        #         f.write("\n")       

        # with open("mix_" + str(args.oppo_type) + "_Animal_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_Animal:
        #         f.write(str(i))
        #         f.write("\n")     
        
        # with open("mix_" + str(args.oppo_type) + "_Outfit_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_Outfit:
        #         f.write(str(i))
        #         f.write("\n") 

        # with open("mix_" + str(args.oppo_type) + "_DogChoosing_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_DogChoosing:
        #         f.write(str(i))
        #         f.write("\n")     

        # with open("mix_" + str(args.oppo_type) + "_SmartPhone_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_SmartPhone:
        #         f.write(str(i))
        #         f.write("\n")     

        # with open("mix_" + str(args.oppo_type) + "_Wholesaler_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        ...
        # with open("mix_" + str(args.oppo_type) + "_Coffee_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_Coffee:
        #         f.write(str(i))
        #         f.write("\n")       

        # with open("mix_" + str(args.oppo_type) + "_Camera_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_Camera:
        #         f.write(str(i))
        #         f.write("\n")     
        
        # with open("mix_" + str(args.oppo_type) + "_FiftyFifty2013_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_FiftyFifty2013:
        #         f.write(str(i))
        #         f.write("\n") 
        
        # with open("mix_" + str(args.oppo_type) + "_Icecream_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_Icecream:
        #         f.write(str(i))
        #         f.write("\n")       

        # with open("mix_" + str(args.oppo_type) + "_Laptop_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_Laptop:
        #         f.write(str(i))
        #         f.write("\n")     
        
        # with open("mix_" + str(args.oppo_type) + "_NiceOrDie_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_NiceOrDie:
        #         f.write(str(i))
        #         f.write("\n") 

        # with open("mix_" + str(args.oppo_type) + "_planes_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_planes:
        #         f.write(str(i))
        #         f.write("\n")     

        # with open("mix_" + str(args.oppo_type) + "_Ultimatum_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_Ultimatum:
        #         f.write(str(i))
        #         f.write("\n")     

        # with open("mix_" + str(args.oppo_type) + "_Amsterdam_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_Amsterdam:
        #         f.write(str(i))
        #         f.write("\n")   
        
        # with open("mix_" + str(args.oppo_type) + "_Barter_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
        #     for i in rewards_Barter:
        #         f.write(str(i))
        #         f.write("\n")     

        # with open("mix_" + str(args.oppo_type) + "_RentalHouse_rewards" + "_seed"  + str(args.seed) + ".txt", "w") as f:      
