...
class Actor_Mix(nn.Module):
    ...
    def pre_squash(self, x, mix_factor, teacher_index):

        # x = F.relu(self.l1(x)).to(self.device)
        h_t1 = self.teacher_actor[teacher_index].l1(x)
        h1 = self.l1(x) * (1 - mix_factor) + h_t1 * mix_factor
        h1 = F.relu(h1).to(self.device)
        h_t1 = F.relu(h_t1).to(self.device)

        # x = F.relu(self.l2(x)).to(self.device)
        h_t2 = self.teacher_actor[teacher_index].l2(h_t1)
        h2 = self.l2(h1) * (1 - mix_factor) + h_t2 * mix_factor
        h2 = F.relu(h2).to(self.device)
        h_t2 = F.relu(h_t2).to(self.device)

        # x = F.relu(self.l3(x)).to(self.device)
        h_t3 = self.teacher_actor[teacher_index].l3(h_t2)
    ...
    def action(self, x, mix_factor, teacher_index, is_deterministic):
        mu, pi, _ = self.pre_squash(x, mix_factor, teacher_index)
        mu = torch.tanh(mu) * self.max_action
        pi = torch.tanh(pi) * self.max_action
        action = mu if is_deterministic else pi
        return action

    def forward(self, x, mix_factor, teacher_index):
        mu, pi, log_std = self.pre_squash(x, mix_factor, teacher_index)
        logp_pi = gaussian_likelihood(pi, mu, log_std)
...
class Critic_Mix(nn.Module):
    ...
    def forward(self, x, u, mix_factor, teacher_index): 
        xu = torch.cat([x, u], 1)

        # x1 = F.relu(self.l1(xu))
        h_t1 = self.teacher_critic[teacher_index].l1(xu)
        h1 = self.l1(xu) * (1 - mix_factor) + h_t1 * mix_factor
        h1 = F.relu(h1)
        h_t1 = F.relu(h_t1)

        # x1 = F.relu(self.l2(x1))
        h_t2 = self.teacher_critic[teacher_index].l2(h_t1)
        h2 = self.l2(h1) * (1 - mix_factor) + h_t2 * mix_factor
        h2 = F.relu(h2)
        h_t2 = F.relu(h_t2)

        # x1 = F.relu(self.l3(x1))
        h_t3 = self.teacher_critic[teacher_index].l3(h_t2)
        h3 = self.l3(h2) * (1 - mix_factor) + h_t3 * mix_factor
        h3 = F.relu(h3)
        h_t3 = F.relu(h_t3)
        x1 = self.l4(h3)
        ...
        # x2 = F.relu(self.l5(xu))
        h_t5 = self.teacher_critic[teacher_index].l5(xu)
        h5 = self.l5(xu) * (1 - mix_factor) + h_t5 * mix_factor
        h5 = F.relu(h5)
        h_t5 = F.relu(h_t5)

        # x2 = F.relu(self.l6(x2))
        h_t6 = self.teacher_critic[teacher_index].l6(h_t5)
        h6 = self.l6(h5) * (1 - mix_factor) + h_t6 * mix_factor
        h6 = F.relu(h6)
        h_t6 = F.relu(h_t6)

        # x2 = F.relu(self.l7(x2))
        h_t7 = self.teacher_critic[teacher_index].l7(h_t6)
        h7 = self.l7(h6) * (1 - mix_factor) + h_t7 * mix_factor
        h7 = F.relu(h7)
        h_t7 = F.relu(h_t7)
        x2 = self.l8(h7)
        return x1, x2

    def Q1(self, x, u, mix_factor, teacher_index):
        xu = torch.cat([x, u], 1)
...
class VNetwork_Mix(nn.Module):
    ...
    def forward(self, x, mix_factor, teacher_index):
        ...
        # x = F.relu(self.l1(x))
        h_t1 = self.teacher_VNetwork[teacher_index].l1(x)
        h1 = self.l1(x) * (1 - mix_factor) + h_t1 * mix_factor
        h1 = F.relu(h1)
        h_t1 = F.relu(h_t1)

        # x = F.relu(self.l2(x))
        h_t2 = self.teacher_VNetwork[teacher_index].l2(h_t1)
        h2 = self.l2(h1) * (1 - mix_factor) + h_t2 * mix_factor
        h2 = F.relu(h2)
        h_t2 = F.relu(h_t2)

        # x = F.relu(self.l3(x))
        h_t3 = self.teacher_VNetwork[teacher_index].l3(h_t2)
        h3 = self.l3(h2) * (1 - mix_factor) + h_t3 * mix_factor
        h3 = F.relu(h3)
        h_t3 = F.relu(h_t3)
...
class SAC_distill(object):
    ...
    def train(self, replay_buffer, iterations, batch_size=64, discount=0.99, tau=0.001, distill_factor=1, teacher_index=0):
        ...
        for it in range(iterations):
            ...
            vf_distill_loss = distill_factor * (F.mse_loss(current_Q1, teacher_Q1[teacher_index]) + F.mse_loss(current_Q2, teacher_Q2[teacher_index]) \
                                                + F.mse_loss(current_V, teacher_V[teacher_index]))
            ...
            actor_distill_loss = distill_factor * (F.mse_loss(mu, teacher_mu[teacher_index]) + F.mse_loss(std, teacher_std[teacher_index]))
...
class SAC_Mix(object):
    ...
    def select_action(self, state, mix_factor=0, teacher_index=1, is_deterministic=False):
        if teacher_index == 0:
            mix_factor = 0
        state = torch.FloatTensor(state.reshape(1, -1)).to(self.device)
        return self.actor.action(state, mix_factor, teacher_index, is_deterministic).cpu().data.numpy().flatten()

    def train(self, replay_buffer, iterations, batch_size=64, discount=0.99, tau=0.001, mix_factor=1, teacher_index=1):
        ...
        for it in range(iterations):
            ...
            # Select action according to policy and add clipped noise
            _, pi, logp_pi = self.actor(state, mix_factor, teacher_index)
            ...
            # Compute the target Q value
            V_ = self.V_network_target(next_state, mix_factor, teacher_index)
            target_Q = reward + (done * discount * V_).detach()

            # Compute the target V value
            Q1_pi, Q2_pi = self.critic(state, pi, mix_factor, teacher_index)
            target_V = torch.min(Q1_pi, Q2_pi)
            target_V = (target_V - alpha * logp_pi).detach()

            # Get current Q/V estimates
            current_Q1, current_Q2 = self.critic(state, action, mix_factor, teacher_index)
            current_V = self.V_network(state, mix_factor, teacher_index)
            ...
            # Compute actor loss
            # _, pi_a, logp_pi_a = self.actor(state)
            Q1_pi = self.critic.Q1(state, pi, mix_factor, teacher_index)
            actor_loss = (alpha * logp_pi - Q1_pi).mean()
