...
class RLAgent(Agent):
    ...
    def train(self, offer_replay_buffer, iterations, batch_size, discount, tau, policy_noise, noise_clip, policy_freq, mix_factor, teacher_index):
        if self.algo == 'sac':#or self.algo == 'sac2'
            if teacher_index == 0:
                mix_factor = 0
            self.offer_policy.train(offer_replay_buffer, iterations, batch_size, discount, tau, mix_factor, teacher_index=teacher_index)
        # elif self.algo == 'td3':
    ...
    def receive(self, last_action):
        if last_action is not None:
            ...
            self.update_obs()
            rl_utility = 0.5 * (int(self.offer_policy.select_action(np.array(self.obs), self.mix_factor, self.teacher_index[0])) + 1) * (1 - self.u_min) + self.u_min
            self.accept = (rl_utility <= utility and self.t <= self.max_round)

    def compute_p(self, mix_factor):
        #print('更新p！')
        sum = 0
        ...
        # while(self.teacher_index[0] == 0):
        #     self.teacher_index = np.random.choice((self.N + 1), 1, p=self.teacher_P)
        self.teacher_index = np.random.choice((self.N + 1), 1, p=self.teacher_P)

    def act(self, mix_factor=0):
        action = self.offer_policy.select_action(np.array(self.obs), mix_factor, self.teacher_index[0])
        return action
