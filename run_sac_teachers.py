...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_models", action="store_true", default=False)    # Whether or not models are saved
    ...
    if args.mode == "train":
        if args.save_models:
            if args.algo == 'sac' and not os.path.exists("./sac_models/tmp/seed{}".format(str(args.seed))):
                os.makedirs("./sac_models/tmp/seed{}".format(str(args.seed)))
            elif args.algo == 'td3' and not os.path.exists("./td3_models/tmp/seed{}".format(str(args.seed))):
                os.makedirs("./td3_models/tmp/seed{}".format(str(args.seed)))
        ...
        while total_timesteps < args.max_timesteps:
            ...
            if episode_num_last_eval >= args.eval_freq:
                ...
                if args.save_models:
                    if args.algo == 'sac':
                        rl_agent.save(directory="./sac_models/tmp/seed{}".format(str(args.seed)))
                    elif args.algo == 'td3':
                        rl_agent.save(directory="./td3_models/tmp/seed{}".format(str(args.seed)))
    ...
    if args.mode == "test":
        print("start testing ...")
        evaluate_policy(rl_agent=rl_agent, opponent=opponent, eval_episodes=10, render=True)
