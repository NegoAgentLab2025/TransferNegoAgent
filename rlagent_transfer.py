...
from algorithm.sac import SAC, Actor
...
class RLAgent(Agent):
    def __init__(self, max_round, name, device, issue_num=3, algo='sac'):
        ...
        self.device = device
        self.N = 4
        self.teacher_policy = [Actor(self.state_dim, self.action_dim, max_action=float(1), device=device)] * (self.N + 1)
        self.teacher_W = [0] * (self.N + 1)
        self.teacher_U = [0] * (self.N + 1)
        self.teacher_P = [0] * (self.N + 1)
        self.tau = 0.05
        self.teacher_index = [0]  # 0代表target policy
        if self.algo == 'sac':
            self.use_automatic_entropy_tuning = True
            self.target_entropy = None
            self.offer_policy = SAC(state_dim=self.state_dim, action_dim=self.action_dim, max_action=float(1), use_automatic_entropy_tuning=self.use_automatic_entropy_tuning, target_entropy=self.target_entropy, device=device)
        # elif self.algo == 'td3':
    ...
    def load(self, label, seed):
        if self.algo == 'sac':
            self.offer_policy.load(filename='SAC', directory="./sac_models/{}/seed{}".format(label, str(seed)))
        # elif self.algo == 'td3':
    ...
    def load_teachers(self):
        for i in range(1, self.N + 1):
            self.teacher_policy[i].load_state_dict(torch.load('./sac_models/transfer/SAC_actor_%s.pth' %i))
            self.teacher_policy[i].eval()
    ...
    def train(self, offer_replay_buffer, iterations, batch_size, discount, tau, policy_noise, noise_clip, policy_freq):
        if self.algo == 'sac':#or self.algo == 'sac2'
            self.offer_policy.train(offer_replay_buffer, iterations, batch_size, discount, tau)
        # elif self.algo == 'td3':
    ...
    def receive(self, last_action):
        if last_action is not None:
            ...
            self.update_obs()
            rl_utility = 0.5 * (self.offer_policy.select_action(np.array(self.obs)) + 1) * (1 - self.u_min) + self.u_min
            self.accept = (rl_utility <= utility and self.t <= self.max_round)

    def compute_p(self):
        #print('更新p！')
        sum = 0
        min_W = min(self.teacher_W)
        max_W = max(self.teacher_W)
        tmp = [0] * (self.N + 1)
        
        if min_W != max_W:
            for i in range(self.N + 1):
                tmp[i] = (self.teacher_W[i] - min_W) / (max_W - min_W)

        for i in range(self.N + 1):
            sum += np.exp(1 * tmp[i])
        for i in range(self.N + 1):
            self.teacher_P[i] = np.exp(1 * tmp[i]) / sum

        self.teacher_index = np.random.choice((self.N + 1), 1, p=self.teacher_P)
        

    def act(self, explo_p = 0, index = [0]):
        # 初始探索阶段随机选择一个teacher 
        if explo_p == 1:
            # print('index:',index)
            temp = torch.FloatTensor(np.array(self.obs).reshape(1, -1)).to(self.device)
            action = self.teacher_policy[index[0]].action(temp, False).cpu().data.numpy().flatten()
        elif explo_p == 0:
            action = self.offer_policy.select_action(np.array(self.obs))
        else:
            # 需要探索并且选中的index不是本身
            if np.random.rand() < explo_p and self.teacher_index[0] != 0:
                #action = self.teacher_policy[self.teacher_index[0]].action(np.array(self.obs), False).cpu().data.numpy().flatten()
                temp = torch.FloatTensor(np.array(self.obs).reshape(1, -1)).to(self.device)
                action = self.teacher_policy[self.teacher_index[0]].action(temp, False).cpu().data.numpy().flatten()
            else:
                action = self.offer_policy.select_action(np.array(self.obs))
        return action
